...
public class Ray {
	...
	for (int i = 0; i < NumOfRays; i++) {
	    ...
	    if (checkSign(nr, nv)) splittedRays.add(new Ray(p0, v));
	}
    ...
    /**
     * compute a random point on a given circle
     * 
	...
	Point3D p = randomPointOnRectangle(centerOfCircle, diameter, diameter);
	double t = p.distance(centerOfCircle);
	while (t > radius) {
	    p = randomPointOnRectangle(centerOfCircle, diameter, diameter);
    ...
    /**
     * compute a random point on a given rectangle
     * 
     * @param centerOfCircle center
     * @param radius         radius of circle
	...
	y = alignZero(y * r);
	if (x != 0) randomCirclePoint = randomCirclePoint.add(firstNormal.scale(x));
	if (y != 0) randomCirclePoint = randomCirclePoint.add(secondNormal.scale(y));
