...
public class BeamOfRaysTests {
	...
	public void SoftShadowTest() {
		...
		scene.geometries.add( //
				new Polygon(new Point3D(-120, -120, 150), new Point3D(120, -120, 150), new Point3D(120, 120, 150),
						new Point3D(-120, 120, 150)).setEmission(new primitives.Color(0, 0, 0))
								.setMaterial(new Material().setKd(0.2).setKs(0.2).setShininess(30)), //
				new Sphere(new Point3D(0, 0, 120), 30).setEmission(new primitives.Color(java.awt.Color.green))
		...
		scene.lights.addAll(List.of(new PointLight(new primitives.Color(700, 400, 400), //
				new Point3D(0, -80, 100)).setkC(1).setkL(4E-4).setkQ(2E-5)));

		ImageWriter imageWriter = new ImageWriter("Soft Shadow Test", 600, 600);
		Render render = new Render().setImageWriter(imageWriter).setCamera(camera)
				.setRayTracer(new RayTracerBasic(scene).setNumOfRay(30)).setMultithreading(3).setDebugPrint();

		render.renderImage();
		render.writeToImage();

		ImageWriter imageWriter2 = new ImageWriter("Soft Shadow Test whitin soft", 600, 600);
	...
	public void DepthOfField() {
		...
		scene.lights.addAll(
				List.of(new SpotLight(new Color(1000, 600, 1000), new Point3D(0, 500, 500), Point3D.ZERO)
						.setkC(1).setkL(0.0001).setkQ(0.00005)
