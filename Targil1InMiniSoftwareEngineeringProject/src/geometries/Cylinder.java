...
public class Cylinder extends Tube {
	...
	if (ints != null) {
	    ...
	    if (alignZero(o1.distance(p.point) - radius) <= 0) p1 = p;
	}
	Point3D o2 = base2.getq0();
	ints = base2.findGeoIntersections(ray, maxDistance);
	if (ints != null) {
	    GeoPoint p = ints.get(0);
	    if (alignZero(o2.distance(p.point) - radius) <= 0) p2 = p;
	}
	if (p1 != null && p2 != null) {
	    p1.geometry = this;
	    p2.geometry = this;
	...
	if (p1 == null && p2 == null) {
		...
		if (distanceFromLowBase <= 0 || alignZero(distanceFromLowBase - height) >= 0) return null;
	    }
	    for (GeoPoint geoPoint : pointIntersectTube) { geoPoint.geometry = this; }
	...
	for (GeoPoint p : pointIntersectTube) {
	    double distanceFromLowBase = alignZero(p.point.subtract(o1).dotProduct(v));
	    if (distanceFromLowBase > 0 && alignZero(distanceFromLowBase - height) < 0) resultList.add(p);
	}
