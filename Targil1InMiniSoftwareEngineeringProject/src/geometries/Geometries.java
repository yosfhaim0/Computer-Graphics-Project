...
public class Geometries extends Intersectable {
	...
	for (Intersectable geo : geometries) {
	    geo.createBox();
	    if (geo.minX < minX) minX = geo.minX;
	    if (geo.maxX > maxX) maxX = geo.maxX;
	    if (geo.minY < minY) minY = geo.minY;
	    if (geo.maxY > maxY) maxY = geo.maxY;
	    if (geo.minZ < minZ) minZ = geo.minZ;
	    if (geo.maxZ > maxZ) maxZ = geo.maxZ;
	...
	if (shapesWhitOutBox != null) geometries.addAll(0, shapesWhitOutBox);
    }
	...
	if (finiteShapes.size() == 1) return finiteShapes;
	...
	while (!finiteShapes.isEmpty()) {
	    ...
	    if (_newShapes == null) _newShapes = new LinkedList<Intersectable>();
	    ...
	    if (finiteShapes.size() == 1) _newShapes.add(finiteShapes.remove(0));
	}
