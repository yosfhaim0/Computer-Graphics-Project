...
public class RayTracerBasic extends RayTracerBase {
	...
	/**
	 * A number of rays for a soft shade or for a glassy
	 */
	private int numOfRayGlossy = 0;
	/**
	 * A number of rays for a soft shade or for a Blurry
	 */
	private int numOfRayBlurry = 0;
	/**
	 ...
	 */
	public RayTracerBasic setNumOfRayGlossy(int numOfRay) {
		this.numOfRayGlossy = numOfRay;
		return this;
	}
	 ...
	 * @return this for chaining
	 */
	public RayTracerBasic setNumOfRayBlurry(int numOfRay) {
		this.numOfRayBlurry = numOfRay;
		return this;
	}
	...
	private Color calcGlobalEffects(GeoPoint geopoint, Vector v, int level, double k) {
		...
		if (kkr > MIN_CALC_COLOR_K) {
			...
			if (numOfRayGlossy > 0) {
				color = calcBeamColor(material.radiusForGlossy, numOfRayGlossy, color, normal, ray, level, kr, kkr);
			} else {
	...
	private double transparency(LightSource light, Vector l, Vector n, GeoPoint geopoint) {
		...
		if (numOfRaySoftShadow > 0) {
			List<Ray> rays = null;
			rays = lightRay.raySplitter(numOfRaySoftShadow, light.getRadius(), lightDistance);
			for (Ray r : rays) {
	 ...
	 */
	private Color calcBeamColor(double radius, int numOfRay, Color color, Vector n, Ray refRay, int level, double k,
			double kk) {
		Color addColor = Color.BLACK;
		List<Ray> rays = null;
		rays = refRay.raySplitter(n, numOfRay, radius, distanceForBlurryGlossy);
		for (Ray ray : rays) {
