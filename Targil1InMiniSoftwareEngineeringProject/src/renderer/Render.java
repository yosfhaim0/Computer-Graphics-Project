...
public class Render {
	...
	public Pixel(int maxRows, int maxCols) {
	    ...
	    if (Render.this.print) System.out.printf("\r %02d%%", this.percents);
	}
	...
	public boolean nextPixel(Pixel target) {
	    ...
	    if (Render.this.print && finishPixels > 0) synchronized (this) {
		notifyAll();
	    }
	    if (finishPixels >= 0) return true;
	    if (Render.this.print) synchronized (this) {
		notifyAll();
	    }
	    return false;
	}
	 ...
	 */
	public void print() {
	    if (Render.this.print) while (this.percents < 100)
		try {
		    synchronized (this) {
			wait();
		    }
	...
	if (raysAliesing != null) rays.addAll(raysAliesing);
	if (raysFocal != null) rays.addAll(raysFocal);
	rays.add(ray);
	Color color;
	if (rays.size() == 1) {
	    ...
	    color = Color.BLACK;
	    for (Ray r : rays) { color = color.add(rayTracerBase.traceRay(r)); }
	...
	if (imageWriter == null)
	    throw new MissingResourceException(RESOURCE_ERROR, RENDER_CLASS, IMAGE_WRITER_COMPONENT);
	if (camera == null) throw new MissingResourceException(RESOURCE_ERROR, RENDER_CLASS, CAMERA_COMPONENT);
	...
	if (imageWriter == null) throw new MissingResourceException("one of the args of Render is null",
		ImageWriter.class.getName(), "imageWriter");
